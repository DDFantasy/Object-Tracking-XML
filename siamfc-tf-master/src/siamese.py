...
from src.crops import extract_crops_z, extract_crops_x, pad_frame, resize_images,extract_crops_x2, pad_frame2
sys.path.append('../')
...
x_sz_ph = tf.placeholder(tf.float64, [hp.scale_num])
...
def build_tracking_graph(final_score_sz, design, env, hp):
    ...
    frame_padded_x, npad_x = pad_frame2(image, frame_sz, pos_x_ph, pos_y_ph, x_sz_ph, avg_chan)
    ...
    x_crops = extract_crops_x2(frame_padded_x, npad_x, pos_x_ph, pos_y_ph, x_sz_ph, design.search_sz)
...
def _match_templates(net_z, net_x, params_names_list, params_values_list):
    ...
    # final is [1, Hf, Wf, BC]
    net_final = tf.concat(tf.split(net_final, hp.scale_num, axis=3), axis=0)
    # final is [B, Hf, Wf, C]
