...
def pad_frame2(im, frame_sz, pos_x, pos_y, patch_sz, avg_chan):
    c = patch_sz[-1] / 2
    xleft_pad = tf.maximum(0, -tf.cast(tf.round(pos_x - c), tf.int32))
    ytop_pad = tf.maximum(0, -tf.cast(tf.round(pos_y - c), tf.int32))
    xright_pad = tf.maximum(0, tf.cast(tf.round(pos_x + c), tf.int32) - frame_sz[1])
...
def extract_crops_x2(im, npad, pos_x, pos_y, sz_src, sz_dst):
    # take center of the biggest scaled source patch
    c = sz_src[-1] / 2
    # get top-right corner of bbox and consider padding
    tr_x = npad + tf.cast(tf.round(pos_x - c), tf.int32)
    ...
    crop = []
    i = 0
    while i < sz_src.get_shape()[0]-1:
        offset = (sz_src[-1]-sz_src[i])/2
        crop_s = tf.image.crop_to_bounding_box(search_area,
                                            tf.cast(offset, tf.int32),
                                            tf.cast(offset, tf.int32),
                                            tf.cast(tf.round(sz_src[i]), tf.int32),
                                            tf.cast(tf.round(sz_src[i]), tf.int32))
        crop_s = tf.image.resize_images(crop_s, [sz_dst, sz_dst], method=tf.image.ResizeMethod.BILINEAR)
        crop.append(crop_s)
        i += 1

    crop_s = tf.image.resize_images(search_area, [sz_dst, sz_dst], method=tf.image.ResizeMethod.BILINEAR)
    crop.append(crop_s)
    crops = tf.stack(crop)
    return crops
