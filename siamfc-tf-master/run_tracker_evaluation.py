...
from src.xml_to_box import xml_to_bbox
...
def main():
    ...
    filename, image, templates_z, scores = siam.build_tracking_graph(final_score_sz, design, env, hp)
    ...
    if evaluation.video == 'all':
        ...
        for i in range(nv):
            if os.path.exists(os.path.join('data/result', videos_list[i])):
                continue
            gt, frame_name_list, frame_sz, n_frames, video_folder, equal = _init_video(env, evaluation, videos_list[i])
            if not equal:
        ...
        gt, frame_name_list, _, _, video_folder, equal = _init_video(env, evaluation, evaluation.video)
        if not equal:
            print("The .jpg and .xml is not equal in", video_folder)
            exit(0)
        pos_x, pos_y, target_w, target_h = xml_to_bbox(gt[evaluation.start_frame])
...
def _compile_results(gt, bboxes, dist_threshold):
    ...
    for i in range(l):
        gt4[i] = xml_to_bbox(gt[i], center=False)
...
def _init_video(env, evaluation, video):
    ...
    file_list = sorted(os.listdir(video_folder))
    frame_name_list = []
    gt_file_list = []
    for f in file_list:
        if f.endswith('.jpg'):
    ...
    gt_file_list = [os.path.join(env.root_dataset, evaluation.dataset, video, '') + s for s in gt_file_list]
    n_frames = len(frame_name_list)
    n_gt = len(gt_file_list)
    equal = True
    if n_frames != n_gt:
        equal = False

    return gt_file_list, frame_name_list, frame_sz, n_frames, video_folder, equal
...
def Draw_Result(Bbox_list, image_list, gt, env):
    for i in range(len(Bbox_list)):
        Bbox = Bbox_list[i]
        Bbox = np.array(Bbox)
        Bbox = Bbox.astype(np.int32)
        gt_ = gt[i]
        Tbox = xml_to_bbox(gt_, center=False)
        image_path = image_list[i]
        foldor_name = gt_.split('/')[-2]
        result_path = os.path.join(env.root_dataset, 'result', foldor_name)
        if not os.path.exists(result_path):
            os.makedirs(result_path)
        image = cv2.imread(image_path)
        image_name = os.path.split(image_path)[-1]
        write_path = os.path.join(result_path, image_name)
        img_cp = copy.copy(image)
        cv2.rectangle(img_cp, (Bbox[0], Bbox[1]), (Bbox[0] + Bbox[2], Bbox[1] + Bbox[3]), (0, 255, 0), 2)
        cv2.rectangle(img_cp, (Tbox[0], Tbox[1]), (Tbox[0] + Tbox[2], Tbox[1] + Tbox[3]), (0, 0, 255), 1)
        cv2.imwrite(write_path, img_cp)
    print "Draw result completed !!!"
